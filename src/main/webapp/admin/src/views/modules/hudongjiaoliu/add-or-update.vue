<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
                        <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'" label="公司编号" prop="gongsibianhao">
          <el-select  @change="gongsibianhaoChange" v-model="ruleForm.gongsibianhao" placeholder="请选择公司编号">
            <el-option
                v-for="(item,index) in gongsibianhaoOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input"v-if="ruleForm.gongsibianhao" label="公司编号" prop="gongsibianhao">
...
export default {
  ...
  methods: {
    ...
    gongsibianhaoChange () {
      this.$http({
        url: `follow/jiazhenggongsi/gongsibianhao?columnValue=`+ this.ruleForm.gongsibianhao,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `hudongjiaoliu/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.hudongjiaoliuCrossAddOrUpdateFlag = false;
                  this.parent.search();
